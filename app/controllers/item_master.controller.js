...
exports.datatableList = (req, res) => {
  ...
  ItemMaster.count({}).exec( (err, c) => {
    recordsTotal = c;
    ItemMaster.count({ $and: querySearchId }).exec((err, c) => {
      recordsFiltered = c;
      if(c == 1){
        start = start - 1;
      }
      ItemMaster.find({  $and: querySearchId }).limit(limit).skip(start).sort({item_code: 'desc'}).exec( (err, results) => {
        if (err) {
          return;
        }
