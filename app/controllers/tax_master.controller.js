...
exports.datatableList = (req, res) => {
  ...
  if(req.body.searchTaxCode){
    querySearchId.push({tax_code: req.body.searchTaxCode });
  }
  ...
  TaxMaster.count({}).exec( (err, c) => {
    recordsTotal = c;
    TaxMaster.count({ $and: querySearchId }).exec((err, c) => {
      recordsFiltered = c;
      if(c == 1){
        start = start - 1;
      }
      TaxMaster.find({  $and: querySearchId }).limit(limit).skip(start).sort({tax_code: 'desc'}).exec( (err, results) => {
        if (err) {
          return;
        }
