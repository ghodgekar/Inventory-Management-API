...
exports.save = (req, res) => {
  ...
  const itemtax = new ItemTaxMaster(reqestData);
  itemtax.save((err, response) => {
    if (err) {
    res.status(500).send({ message: err });
    return;
    }else {
    res.status(200).send({ data: response, message: "Data Saved Successfully In itemtax Master" });
    return;    
    }
  });
};
...
exports.update = (req, res) => {
  ...
  ItemTaxMaster.findByIdAndUpdate({_id:reqestData._id},reqestData,{ new: true },(err, response) => {
    if (err) {
      res.status(500).send({ message: err });
      return;
    }else{
      res.status(200).send({ data:response, message: "Data Updated Successfully In itemtax Master"  });
      return;
    }
  });
};
...
exports.delete = (req, res) => {
  let reqestData = req.body;
  ItemTaxMaster.findByIdAndUpdate({_id:reqestData._id}, {status : 'Inactive'} ,{ new: true },(err, response) => {
    if (err) {
      res.status(500).send({ message: err });
      return;
    }else{
      res.status(200).send({ message: "Data Deleted In itemtax Master"  });
      return;
    }
  });
};
...
exports.datatableList = (req, res) => {
  ...
  if(req.body.searchItemCode){
    querySearchId.push({item_code: req.body.searchItemCode });
  }
  ...
  ItemTaxMaster.count({}).exec( (err, c) => {
    recordsTotal = c;
    ItemTaxMaster.count({ $and: querySearchId }).exec((err, c) => {
      recordsFiltered = c;
      if(c == 1){
        start = start - 1;
      }
      ItemTaxMaster.find({  $and: querySearchId }).limit(limit).skip(start).sort({item_code: 'desc'}).exec( (err, results) => {
        if (err) {
          return;
        }
